...
def get_ode(ode_name, param):
    if ode_name == 'SineWave':
        ...
        ode = FracODE(param)
    elif ode_name == 'TrigonometricODE':
        ode = TrigonometricODE(param)
    else:
...
class TrigonometricODE(ODE):
    ...
    def __init__(self, param=None):
        super().__init__(1, param)
        self.rho, self.omega = self.param
        self.init_high = 0.01
        if self.has_coef:
            self.T = 2*np.pi
        else:
            self.T = 2*np.pi
        self.name = 'TrigonometricODE'
        self.std_base = 1.0

    def _dx_dt(self, X):
        dxdt = self.rho * np.sin(self.omega * X)  
        return [dxdt]
    ...
    def functional_theta(self, theta):
        assert len(theta) == 2
        new_ode = TrigonometricODE(theta)
        return new_ode.dx_dt_batch
