...
#rm ../results/SelkovODE-noise_sigma.txt

for noise in "${noise_arr[@]}"
do
    #python -u ../evaluation.py --ode_name=${ode} --noise_sigma=${noise} --n_sample=${sample} --freq=${freq} --alg=diff >> ../results/SelkovODE-noise_sigma.txt 
    python -u ../evaluation.py --ode_name=${ode} --noise_sigma=${noise} --n_sample=${sample} --freq=${freq} --alg=vi >> ../results/SelkovODE-noise_sigma.txt
done

cat ../results/SelkovODE-noise_sigma.txt
