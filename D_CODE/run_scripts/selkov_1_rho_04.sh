...
#rm ../results/SelkovODE-noise_rho_04.txt

for noise in "${noise_arr[@]}"
do
    python -u ../evaluation.py --ode_name=${ode} --noise_sigma=${noise} --n_sample=${sample} --freq=${freq} --alg=diff >> ../results/SelkovODE-noise_rho_04.txt 
    #python -u ../evaluation.py --ode_name=${ode} --noise_sigma=${noise} --n_sample=${sample} --freq=${freq} --alg=vi >> ../results/SelkovODE-noise_rho_04.txt
done

cat ../results/SelkovODE-noise_rho_04.txt
