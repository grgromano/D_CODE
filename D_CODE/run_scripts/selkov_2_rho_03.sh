...
#rm ../results/SelkovODE-noise-1_rho_03.txt

for noise in "${noise_arr[@]}"
do
    python -u ../evaluation.py --ode_name=${ode} --noise_sigma=${noise} --x_id=${x_id} --n_sample=${sample} --freq=${freq} --alg=diff >> ../results/SelkovODE-noise-1_rho_03.txt 
    #python -u ../evaluation.py --ode_name=${ode} --noise_sigma=${noise} --x_id=${x_id} --n_sample=${sample} --freq=${freq} --alg=vi >> ../results/SelkovODE-noise-1_rho_03.txt
done

cat ../results/SelkovODE-noise-1_rho_03.txt
