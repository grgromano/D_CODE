...
ode=SelkovODE_rho_06
x_id=1

# running with freq=10, T=20
...
#rm ../results/SelkovODE-noise-1_rho_06.txt

for noise in "${noise_arr[@]}"
do
    python -u ../evaluation.py --ode_name=${ode} --noise_sigma=${noise} --x_id=${x_id} --n_sample=${sample} --freq=${freq} --alg=diff >> ../results/SelkovODE-noise-1_rho_06.txt 
    #python -u ../evaluation.py --ode_name=${ode} --noise_sigma=${noise} --x_id=${x_id} --n_sample=${sample} --freq=${freq} --alg=vi >> ../results/SelkovODE-noise-1_rho_06.txt
done

cat ../results/SelkovODE-noise-1_rho_06.txt
