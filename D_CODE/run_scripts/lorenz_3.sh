...
#rm results/Lorenz-noise-2.txt

for noise in "${noise_arr[@]}"
do
    python -u ../evaluation.py --ode_name=${ode} --noise_sigma=${noise} --n_sample=${n_sample} --freq=${freq} --x_id=${x_id} --alg=diff >> ../results/Lorenz-noise-2.txt
    python -u ../evaluation.py --ode_name=${ode} --noise_sigma=${noise} --n_sample=${n_sample} --freq=${freq} --x_id=${x_id} --alg=vi >> ../results/Lorenz-noise-2.txt
done
